...
class QuizParser(xml.sax.ContentHandler):
	"""
	The QuizParser class loads a aprticular quiz file, parses it, and returns a fully built
	Quiz object  that can then be presented to the user.
	"""
	...
	def parse_quiz(self, quizpath):		#quizpath is the xml file needed to parse
		#load file contents
		quiztext = ""
		...
		#TODO: parse the file
		xml.sax.parseString(quiztext, self)		#here self identifies as the QuizParser class where all the parsing action is done
	...
	def endElement(self, tagname):
		if tagname == "QuizML":
			self._parse_state = QuizParserState.IDLE
		#TODO: process rest tof the tags
		elif tagname == "Description":
			self._parse_state = QuizParserState.PARSE_QUIZ
		elif tagname == "Question":
			self.new_quiz.questions.append(self._current_question)
			self._parser_state = QuizParserState.PARSE_QUIZ
		elif tagname == "QuestionText":
			self._parse_state = QuizParserState.PARSE_QUESTION
		elif tagname == "Answer":
